      ...
      <tbody>
        @forelse($salas as $sala)
          <tr class="border-b border-gray-50 hover:bg-gray-50 transition-colors">
            <td class="p-4 text-gray-800 font-medium">{{ $sala->nome }}</td>
            <td class="p-4 text-gray-600">Bloco {{ $sala->bloco }} - {{ $sala->piso === 0 ? 'Térreo' : $sala->piso . 'º andar' }}</td>
            <td class="p-4 text-gray-600">{{ $sala->capacidade }} pessoas</td>
            <td class="p-4">
              @if($sala->status_sala === 'disponivel')
                <span class="px-2 py-1 bg-green-100 text-green-700 rounded-full text-xs font-bold uppercase">Disponível</span>
              @elseif($sala->status_sala === 'manutencao')
            ...
            <td class="p-4 text-right space-x-2">
              <a href="{{ route('salas.edit', $sala) }}" class="text-blue-600 hover:text-blue-800 font-medium text-sm">Editar</a>
              <form action="{{ route('salas.destroy', $sala) }}" method="POST" class="inline" onsubmit="return confirm('Tem certeza que deseja apagar esta sala?')">
                @csrf
                @method('DELETE')
        ...
        @empty
          <tr>
            <td colspan="5" class="p-8 text-center text-gray-500">Nenhuma sala cadastrada.</td>
          </tr>
        @endforelse
      </tbody>
    </table>
