<x-app-layout>
  ...
  <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-2xl">
    <h2 class="text-2xl font-bold text-gray-800 mb-6">Cadastrar Nova Sala</h2>
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select name="status_sala" id="status_sala" required class="w-full rounded-lg border-gray-300 focus:border-[#76A068] focus:ring-[#76A068] shadow-sm">
            <option value="disponivel" {{ old('status_sala') == 'disponivel' ? 'selected' : '' }}>Disponível</option>
            <option value="manutencao" {{ old('status_sala') == 'manutencao' ? 'selected' : '' }}>Em Manutenção</option>
            <option value="inativo" {{ old('status_sala') == 'inativo' ? 'selected' : '' }}>Inativo</option>
          </select>
